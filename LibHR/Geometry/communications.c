...
#endif
{
  ...
  for (i = 0; i < gd->ncopies_gauge; ++i) {
    ...
    memcpy(((gf->ptr) + 6 * gd->copy_to[i]), ((gf->ptr) + 6 * gd->copy_from[i]),
           6 * (gd->copy_len[i]) * sizeof(*(gf->ptr)));
    /*
         for(j=0; j<gd->copy_len[i]; j++) {
         x=gd->copy_from[i]+j;
         y=gd->copy_to[i]+j;
...
static void sync_gauge_transf(suNg_field *gf) {
  ...
  for (i = 0; i < gd->ncopies_gauge; ++i) {
    memcpy(((gf->ptr) + gd->copy_to[i]), ((gf->ptr) + gd->copy_from[i]),
           (gd->copy_len[i]) * sizeof(*(gf->ptr)));
  }
}
...
#endif
{
  ...
  if (nreq > 0) {
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      error(1, 1, "complete_clover_force_sendrecv " __FILE__,
  ...
  if (gf_control > 0) {
    ...
    lprintf("MPI TIMING", 0,
            "complete_clover_force_sendrecv" __FILE__ " %ld sec %ld usec\n",
...
#endif
{
...
#ifdef MPI_TIMING
  error(gf_control > 0, 1, "start_clover_force_sendrecv " __FILE__,
  ...
  for (i = 0; i < (gd->nbuffers_gauge); ++i) {
    /* send ith buffer */
    mpiret =
        MPI_Isend((double *)((gf->ptr) + 6 * gd->sbuf_start[i]), /* buffer */
                  (gd->sbuf_len[i]) * sizeof(suNf) / sizeof(double) *
                      6,                 /* lenght in units of doubles */
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      MPI_Error_string(mpiret, mesg, &mesglen);
      lprintf("MPI", 0, "ERROR: %s\n", mesg);
      error(1, 1, "start_clover_force_sendrecv " __FILE__,
    ...
    /* receive ith buffer */
    mpiret = MPI_Irecv(
        (double *)((gf->ptr) + 6 * gd->rbuf_start[i]), /* buffer */
        (gd->rbuf_len[i]) * sizeof(suNf) / sizeof(double) *
            6,                     /* lenght in units of doubles */
        MPI_DOUBLE,                /* basic datatype */
...
void complete_gt_sendrecv(suNg_field *gf) {
  ...
  if (nreq > 0) {
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      error(1, 1, "complete_gt_sendrecv " __FILE__,
            "Cannot complete communications");
    }
#endif
  }
...
void start_gt_sendrecv(suNg_field *gf) {
  ...
  for (i = 0; i < (gd->nbuffers_gauge); ++i) {
    /* send ith buffer */
    mpiret =
        MPI_Isend((double *)((gf->ptr) + gd->sbuf_start[i]), /* buffer */
                  (gd->sbuf_len[i]) * sizeof(suNg) /
                      sizeof(double),    /* lenght in units of doubles */
                  MPI_DOUBLE,            /* basic datatype */
                  gd->sbuf_to_proc[i],   /* cid of destination */
                  i,                     /* tag of communication */
    ...
    /* receive ith buffer */
    mpiret = MPI_Irecv(
        (double *)((gf->ptr) + gd->rbuf_start[i]), /* buffer */
        (gd->rbuf_len[i]) * sizeof(suNg) /
            sizeof(double),        /* lenght in units of doubles */
        MPI_DOUBLE,                /* basic datatype */
        gd->rbuf_from_proc[i],     /* cid of origin */
        i,                         /* tag of communication */
...
void complete_sc_sendrecv(suNg_scalar_field *sf) {
  ...
  if (nreq > 0) {
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      error(1, 1, "complete_sc_sendrecv " __FILE__,
            "Cannot complete communications");
    }
#endif
  }
  ...
  if (sf_control > 0) {
    ...
    timeval_subtract(&sfetime, &sfend, &sfstart);
    lprintf("MPI TIMING", 0,
            "complete_sc_sendrecv" __FILE__ " %ld sec %ld usec\n",
            sfetime.tv_sec, sfetime.tv_usec);
    sf_control = 0;
  }
#endif
...
void start_sc_sendrecv(suNg_scalar_field *sf) {
...
#ifdef MPI_TIMING
  error(sf_control > 0, 1, "start_sc_sendrecv " __FILE__,
        "Multiple send without receive");
  gettimeofday(&sfstart, 0);
  sf_control = 1;
#endif
  ...
  for (i = 0; i < (gd->nbuffers_spinor); ++i) {
    ...
    mpiret = MPI_Isend(
        (double *)((sf->ptr) + (gd->sbuf_start[i]) -
                   (gd->master_shift)), /* buffer */
        (gd->sbuf_len[i]) * (sizeof(suNg_vector) /
                             sizeof(double)), /* lenght in units of doubles */
        MPI_DOUBLE,                           /* basic datatype */
        gd->sbuf_to_proc[i],                  /* cid of destination */
        i,                                    /* tag of communication */
    ...
    mpiret = MPI_Irecv(
        (double *)((sf->ptr) + (gd->rbuf_start[i]) -
                   (gd->master_shift)), /* buffer */
        (gd->rbuf_len[i]) * (sizeof(suNg_vector) /
                             sizeof(double)), /* lenght in units of doubles */
        MPI_DOUBLE,                           /* basic datatype */
        gd->rbuf_from_proc[i],                /* cid of origin */
        i,                                    /* tag of communication */
